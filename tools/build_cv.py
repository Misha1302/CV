...
def build_block(profile: dict) -> str:
    ...
    for date, title, bullets in profile["experience"]:
        ...
        experience.append(f'<article class="pcv-entry"><div class="pcv-date">{esc(date)}</div><div><h3>{esc(title)}</h3><ul>{items}</ul></div></article>')
    projects = "".join(f'<article class="pcv-project"><div class="pcv-project-head"><h3>{esc(title)}</h3></div><p>{esc(text)}</p></article>' for title, text in profile["projects"])
    skills = "".join(f'<div class="pcv-skill"><strong>{esc(title)}</strong><span>{esc(text)}</span></div>' for title, text in profile["skills"])
    return f"""<div class="print-cv" aria-label="Focused one-page CV"><header class="pcv-header"><div><h1>{esc(profile["name"])}</h1><h2>{esc(profile["role"])}</h2></div><div class="pcv-contact">{contact}</div></header><p class="pcv-summary">{esc(profile["summary"])}</p><div class="pcv-proofs">{proofs}</div><div class="pcv-columns"><main class="pcv-main"><section><h2 class="pcv-section-title">{labels["experience"]}</h2>{''.join(experience)}</section><section><h2 class="pcv-section-title">{labels["projects"]}</h2>{projects}</section></main><aside class="pcv-side"><section><h2 class="pcv-section-title">{labels["skills"]}</h2>{skills}</section><section class="pcv-compact"><h2 class="pcv-section-title">{labels["education"]}</h2><p>{esc(profile["education"])}</p></section><section class="pcv-compact"><h2 class="pcv-section-title">{labels["recognition"]}</h2><p>{esc(profile["recognition"])}</p></section><section class="pcv-compact"><h2 class="pcv-section-title">{labels["communication"]}</h2><p>{esc(profile["communication"])}</p></section><section class="pcv-compact"><h2 class="pcv-section-title">{labels["availability"]}</h2><p>{esc(profile["availability"])}</p></section></aside></div></div>"""
...
def render(pages: list[tuple[Path, Path]], evidence_dir: Path | None) -> None:
    with sync_playwright() as p:
        ...
        for html_path, pdf_path in pages:
            ...
            metrics = page.eval_on_selector(".print-cv", "el => ({client: el.clientHeight, scroll: el.scrollHeight, width: el.scrollWidth, clientWidth: el.clientWidth})")
...
def main() -> None:
    ...
    for filename, profile in data["profiles"].items():
        ...
        block = build_block(profile)
        inject(html_path, block, args.write_html)
        if args.check_html:
            inject(html_path, block, False)
        if args.render_dir:
            pages.append((html_path, args.render_dir / profile["pdf"]))
    if pages:
        render(pages, args.evidence_dir)
        report = [validate_pdf(pdf) for _, pdf in pages]
        if args.report:
