...
# Cache-bust the public surface consistently.
...
# Natural Russian wording for the C++ systems profile, including the focused PDF source.
ru_cpp = Path('ru-cpp-systems.html')
ru_cpp_replacements = {
    'C++23, C17, анализ программ, NASM IA-32, x86-64 codegen, Linux и строгий toolchain.':
        'C++23, C17, анализ программ, NASM IA-32, генерация x86-64-кода, Linux и строгая сборка.',
    '<span>C++ systems · program analysis</span>': '<span>C++ systems · анализ программ</span>',
    '<strong>C17 analyzer</strong><span>нормализация + exact/conservative strategies</span>':
        '<strong>C17-анализатор</strong><span>нормализация, точные и консервативные стратегии</span>',
    '<strong>C++23 graph core</strong><span>RPO, shortest paths, SCC, max-flow</span>':
        '<strong>C++23-ядро графов</strong><span>RPO, кратчайшие пути, SCC, максимальный поток</span>',
    '<p class="org">Compiler/runtime architecture · verification engineering</p>':
        '<p class="org">Архитектура compiler/runtime · проверка корректности</p>',
    '<span class="project-type">C17 · Python · program analysis</span>':
        '<span class="project-type">C17 · Python · анализ программ</span>',
    'точный аффинный анализ, monotonic scan и ограниченный перебор.':
        'точный аффинный анализ, монотонный проход и ограниченный перебор.',
    '<strong>Strict toolchain</strong><span>CMake, -Werror, ASan/UBSan, clang-tidy</span>':
        '<strong>Строгая сборка</strong><span>CMake, -Werror, ASan/UBSan, clang-tidy</span>',
    'Разработал общее ядро ориентированного графа и компоненты RPO с обнаружением циклов, Dijkstra, Tarjan SCC и Dinic.':
        'Разработал ядро ориентированного графа, RPO с обнаружением циклов, алгоритм Дейкстры, SCC Тарьяна и максимальный поток Диница.',
    ...
    'графы, деревья, строки, структуры данных, max-flow, SCC и кратчайшие пути':
        'графы, деревья, строки, структуры данных, максимальный поток, SCC и кратчайшие пути',
    'отдельный x86-64 SysV emitter с анализом живости, распределением регистров, iced-x86 и изолированным запуском нативного кода.':
        'отдельный генератор x86-64 SysV-кода с анализом живости, распределением регистров, iced-x86 и изолированным запуском нативного кода.',
}
...
# Small English clarity fixes in focused low-level skill lines.
for name in ('en-compiler.html', 'en-cpp-systems.html'):
    path = Path(name)
    text = path.read_text(encoding='utf-8')
    text = text.replace('cdecl, stack/x87;', 'cdecl, stack frames and x87;')
    path.write_text(text, encoding='utf-8')

# Replace compact recognition paragraphs in all four focused PDF sources.
for name in ('ru-compiler.html', 'ru-cpp-systems.html'):
    replace_regex(
        Path(name),
        r'(<section class="pcv-compact"><h2 class="pcv-section-title">Достижения</h2><p>).*?(</p></section>)',
        rf'\1{RU_ACHIEVEMENT}\2',
    )
for name in ('en-compiler.html', 'en-cpp-systems.html'):
    replace_regex(
        Path(name),
        r'(<section class="pcv-compact"><h2 class="pcv-section-title">Recognition</h2><p>).*?(</p></section>)',
        rf'\1{EN_ACHIEVEMENT}\2',
    )

# Raise the actual minimum print font without shrinking or adding another page.
...
# Release metadata and review files.
renames = {
...
readme_text = readme_text.replace(
    '# Mikhail Razakov - targeted CV variants v28',
...
readme_text = readme_text.replace(marker, v29_section + marker, 1)
readme_text = readme_text.replace(
...
metadata_text = metadata_text.replace('- Version: v28', '- Version: v29', 1)
...
for name in (
    ...
    path = Path(name)
    text = path.read_text(encoding='utf-8').replace('v28', 'v29')
    path.write_text(text, encoding='utf-8')
