...
replace_exact(
    ...
    '<li>Developed a shared graph core and C++23 analysis components for the LLVM track.</li><li>Implemented RPO with cycle detection, Dijkstra, Tarjan SCC and Dinic; used CMake, warnings-as-errors, ASan/UBSan, clang-tidy, explicit includes and reproducible I/O tests.</li>',
)
replace_exact(
    en,
...
replace_pairs = (
    ("- Version: v29", "- Version: v30"),
    ("- Date: 2026-07-24", "- Date: 2026-07-25"),
    ("- Previous repository baseline: v28, commit `d655f6854a1d5ac8d87393cc1f4c82567ac1ddee`", "- Previous repository baseline: v29, commit `fae03e5adb4152bd1aaa43e56f49d383dc8f9e6d`"),
    ("- Rebuilt PDFs: compiler RU/EN and C++ systems RU/EN", "- Rebuilt PDFs: compiler RU/EN"),
)
for old, new in replace_pairs:
    if old not in text:
        raise RuntimeError(f"RELEASE-METADATA missing: {old}")
...
for old_name, new_name in (("CONTENT-REVIEW-v29.md", "CONTENT-REVIEW-v30.md"), ("QA-report-targeted-cv-v29.md", "QA-report-targeted-cv-v30.md")):
    old = Path(old_name)
    new = Path(new_name)
...
The Russian focused CV localizes section labels while preserving conventional compiler identifiers. Recognition and technical communication are compact supporting evidence; technical work remains dominant.

Verdict: suitable for focused first-contact compiler applications after automated and human render validation.
""",
    encoding="utf-8",
)
Path("QA-report-targeted-cv-v30.md").write_text("# QA report - targeted CV v30\n\nFinalized by CI.\n", encoding="utf-8")
...
# Source postconditions with language-specific scopes.
ru_text = ru.read_text(encoding="utf-8")
en_text = en.read_text(encoding="utf-8")
for stale in ("baseline от 14.07.2026", "verified baseline dated July 14, 2026", "Студент НИУ ВШЭ.", "HSE University student.", "<strong>Конвейер</strong>Лексер"):
...
print("Compiler CV v30 source hardening v2 applied successfully")
